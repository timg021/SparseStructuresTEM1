...
//-----------------------------------------------------------------
void SaveXYZfile(string fileout, float ctblength, float ctblengthz, int natom, int* Znum, float* x, float* y, float* z, float* occ, float* wobble)
{
	// Write out the modified XYZ file with added ice
	string filetest = fileout;// [0] ;
	filetest.replace(filetest.find_last_of(".") + 1, 3, "xyz");
...
// this function assumes that the input structure comes enclosed in the cube ctblength^3
{
	...
	//  copy PDB coord. back from into original simple format
	free(x); free(y); free(z); free(occ); free(wobble); free(Znum);
	x = (float*)malloc1D(ntotal, sizeof(float), "x"); *px = x;
	y = (float*)malloc1D(ntotal, sizeof(float), "y"); *py = y;
	z = (float*)malloc1D(ntotal, sizeof(float), "z"); *pz = z;
	occ = (float*)malloc1D(ntotal, sizeof(float), "occ"); *pocc = occ;
	wobble = (float*)malloc1D(ntotal, sizeof(float), "wobble"); *pwobble = wobble;
	Znum = (int*)malloc1D(ntotal, sizeof(int), "Znum"); *pZnum = Znum;
	for (int i = 0; i < natom; i++) {
