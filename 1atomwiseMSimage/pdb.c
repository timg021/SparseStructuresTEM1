 ...
 */
void data_from_VestaXYZfile(char* pdbfname, pdbdata* pd)
{
	...
	if (file != NULL) {
		...
		while (fgets(line, sizeof line, file) != NULL) 
		{
			sscanf(line, "%s %f %f %f", &elem, &x, &y, &z);
			strcpy(pd->adata[count].element, elem);
			pd->adata[count].x = x;
			pd->adata[count].y = y;
			pd->adata[count].z = z;
			pd->adata[count].occupancy = 1.00000;
			count++;
		}

		if (count != natoms) printf("!!!wrong number of atom lines (in data_from_VestaXYZfile).\n");
